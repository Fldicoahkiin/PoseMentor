...
start_services() {
  ...
  nohup uv run python "$PROJECT_ROOT/admin_console.py" >"$LOG_DIR/admin_console.log" 2>&1 &
  echo $! >"$PID_DIR/admin_console.pid"

  nohup uv run python "$PROJECT_ROOT/app_demo.py" >"$LOG_DIR/app_demo.log" 2>&1 &
  echo $! >"$PID_DIR/app_demo.pid"

  echo "服务已启动"
  echo "- Backend API:  http://127.0.0.1:8787"
  echo "- 管理后台:      http://127.0.0.1:7861"
  echo "- 在线系统:      http://127.0.0.1:7860"
}

stop_services() {
  ensure_dirs
  for service in backend_api admin_console app_demo; do
